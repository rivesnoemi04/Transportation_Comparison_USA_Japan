...
ghg_data <- read_csv("G:/apartin/GES4070 GEOVis/Project2/data/emmisionsdata.csv")
...
# Filter for Luxembourg and 2022
luxembourg_2022 <- ghg_data %>%
  filter(Entity == "Luxembourg", Year == 2022) %>%
  select(buildings, industry, `land use and forestry`, `fuel combustion`,
         transportation, `manufacturing and construction`, `energy production`,
         `electricity and heat`, `bunker fuels`)

# Convert to long format for ggplot
luxembourg_2022_long <- luxembourg_2022 %>%
  pivot_longer(cols = everything(),
               names_to = "sector",
               values_to = "emissions")

# Optional: highlight Transportation
luxembourg_2022_long <- luxembourg_2022_long %>%
  mutate(highlight = ifelse(sector == "transportation", "Transportation", "Other"))


ggplot(luxembourg_2022_long, aes(x = reorder(sector, -emissions), y = emissions, fill = sector)) +
  geom_bar(stat = "identity", color = "black", size = 0.7) +
  scale_fill_brewer(palette = "YlGn", direction = 1) +  # Yellow-Green palette
  labs(title = "Luxembourg Greenhouse Gas Emissions by Sector (2022)",
       x = "Sector",
       y = "Emissions (metric tons CO2e)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
